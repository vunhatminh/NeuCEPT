...
#             perturb_dict[method][sample][0] = torch.clip(perturb_dict[method][sample][0] + weighted_noise, 
#                                                       min=d_min, max=d_max)
...
def get_perturb_accuracy(args, model, data_loader, device,
    ...
    with torch.no_grad():
        ...
        for X, y_true in data_loader:
            ...
            perturbs = gen_perturb(args, X, 
                       noise = noise_,
                       gamma = gamma_)
            for method in args.explainer:
                perturbs[method] = perturbs[method].to(device)
                y_hat = model(perturbs[method])
...
for noise in noise_levels:
    corrects = get_perturb_accuracy(prog_args, net, train_loader, DEVICE,
